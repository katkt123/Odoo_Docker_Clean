...
This project is configured to use a single fixed Postgres database: `Odoo`.

- Config: `config/odoo.conf` sets `db_name=Odoo`, `dbfilter=^Odoo$`, and `list_db=False`.
- Result: there is no database selector screen; all web requests go to the same DB.

When you clone/pull this repo on another machine, you must also create/restore the database `Odoo`.
...
Git does not include database data. To share the same website content, export a dump from the source machine and restore it into database `Odoo`.
